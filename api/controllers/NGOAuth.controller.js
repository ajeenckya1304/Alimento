...
const sendVerificationEmail = async (email, otp) => {
  ...
  let mailOptions = {
    ...
    text: `Your OTP for AliMento signup is: ${otp}`,
...
export const NGOsignup = async (req, res, next) => {
  ...
  try {
    ...
    const existingUser = await NGOUser.findOne({ email });
    if (existingUser) {
    ...
    const newUser = new NGOUser({ username, email, password: hashedPassword, address });
    await newUser.save();
...
export const NGOUpdatePassword = async (req, res, next) => {
...
export const google = async (req, res, next) => {
  try {
    const user = await NGOUser.findOne({ email: req.body.email });
    if (user) {
...
export const NGOsignOut = async (req, res, next) => {
  try {
